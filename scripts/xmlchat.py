...
modified by benjamin boerschinger, 19th July, 2012

now, the script also returns the time-stamps associated with every utterance, required
for talks such as forced alignment
"""
...
def _start_element(name, attrs):
    ...
    if name == "w" or (name in _morph_features and _morph != None):
        ...
        _utterance["start"] = float(attrs["start"])
        _utterance["end"] = float(attrs["end"])
